...
include "../includes/conexion.php";
	...
	$id_grupo=$_REQUEST['cod_grupo'];
	$idgrupom5=$_REQUEST['cod_grupom5'];
	$id_clase=$_REQUEST['cod_clase'];
	$hora_ini=$_REQUEST['hora_ini'];
	$hora_fin=$_REQUEST['hora_fin'];
	$t_tiempo=$_REQUEST['tipo_tiempo'];
	$sub_grupo=$_REQUEST['sub_grupo'];
    ...
    $query_nbgru= $bdcon->prepare("SELECT grupo FROM aca_registroestmat WHERE codest='$codest' and periodo='202201'");
    $query_nbgru->execute();
	...
	// 	# no se verifica hora
	// }else{
		// verificamos si esta dentro de la hora para la evaluacion 
    	if ($hora_fin<"00:15:00") {
    		# code...
    	}else{
    		if ( ($hora_ini<=$hr_act) && ($hora_fin>=$hr_act) ) {	   
		    }else{
		    ?>
		        <div class="alert alert-danger" align="center">¡Tiempo expirado trato de guardar a hrs. <?=$hr_act?>!<br>
		        y su hora de finalización era a hrs. <?=$hora_fin?> </div>
		        <br>
		        <br>
		    <?php
	...
	if($query_exis->rowCount()>0){
		...
		<?php
			...
			foreach ($preg as $key => $value) {
				...
				if (is_array($resp)) {
					...
					$query_insert_m5d = $bdcon->prepare("INSERT INTO aa_clases_virtuales_m5_respuesta_deta VALUES ('0','$cod_eval','$codest','$cod_ban','$value','0','0.00','Sin respuesta')");
						$query_insert_m5d->execute(); 
					$sum_resp=$sum_resp+0;
				}			
				$cont_preg++;
		...
		if($sub_grupo==0){
			...
			$notaf=round(($sum_resp*3)/10,2);
		}
		$query_insert_m5 = $bdcon->prepare("INSERT INTO aa_clases_virtuales_m5_respuestas VALUES ('0','$cod_eval','$codest','$cod_ban','$notaf',DATE_FORMAT(now(),'%Y-%m-%d'),DATE_FORMAT(now(),'%H:%i:%s'),'$idgrupom5','$sub_grupo')");
		$query_insert_m5->execute();
		if ($query_insert_m5) {
			?>
		    <div class="alert alert-success" align="center">			 
			   	<strong>Correcto! </strong> <font>se guardó correctamente la evaluación y nota.</font>
			</div>
			<?php
		}else{
			?>
