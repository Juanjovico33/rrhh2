...
if($sub_grupo==0){
            ...
            while ($row = $class->fetch(PDO::FETCH_ASSOC)) {
                ...
                <form action="#" id="form<?=$id_clase;?>">
                    ...
                    <div id="msjm2<?=$id_clase; ?>">
                        <tr><button class="btn btn-success" onclick="set_M2preguntaforo(<?=$grupo_aux?>, <?=$id_grupo?>, <?=$id_clase?>, <?=$cod_est?>, <?=$row['cod']?>, '<?=$nbgrupo?>')">PREGUNTAR</button></tr>
                    </div>
    ...
    if($tipo_sub==2){
        echo "<div class='alert alert-success'><b>Investigación</b> no requiere de esta participación!</div>";
    }else{
               ...
               <table class="table">
                    ...
                    if($obj_tar->rowcount()>0){
                        ...
                        if ($fec_act>=$fec_desde && $fec_act<=$fec_pub_res) {
                            if ($res_m4->rowcount()>0) {
                                ...
                                if($_switch!=0){
                                    ...
                                    $desabil="disabled='disabled'";
                                    $msj_m4="<div class='alert alert-warning alert-danger'>No puede participar del momento 4 por que no tiene registrada su asistencia en esta clase.</div>";
                                }
                            }
                        }else{
                            $desabil="disabled='disabled'";
                        ...
                        if ($tipo_resp=='1') {
                            ...
                            <tr>
                                <td><div style="text-align:center;"id="msj_ta<?=$cod_ta; ?>"><button class="btn btn-success" onclick="set_resumen_m4(<?=$grupo_aux?>, <?=$id_grupo?>, <?=$cod_ta?>,<?=$cod_est;?>,'<?=$nbgrupo;?>')" <?=$desabil;?>>GUARDAR RESUMEN</button> <?=$msj_m4;?> </div></td>
                            </tr>
                            <?php
                        }else{
                            if ($tipo_resp=='2') {
                ...
                if ($fec_act==$fec_pub_eval) {
                    if($_switch!=0 || $tipo_sub==2){
                        ...
                        if($tipo_sub==2){
                            $disab="";
                            $func="iniciar_eval";
                        }else if($horario->puede_to_M5($_switch)==1){
                            $disab="";
                            $func="iniciar_eval";
                        }else{
                            $disab="disabled='disabled'";
                            $func="alert('Fuera del tiempo para participar!')";
                            $msj_m5="<div class='alert alert-warning alert-danger'>No se puede registrar, su asistencia está fuera del tiempo permitido. Para poder participar de los momentos debe de ingresar en los 10 minutos iniciales desde la hora de inicio de su clase.</div>";
                        }
                    }else{
                        $disab="disabled='disabled'";
                        $func="alert('No tiene asistencia!')";
                        $msj_m5="<div class='alert alert-warning alert-danger'>No puede participar del momento 5 por que no tiene registrada su asistencia en esta clase.</div>";
                    }
